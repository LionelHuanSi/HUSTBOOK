            ...
            switch (choice) {
                ...
                case 2:
                    ...
                    try {
                    	...
                    	String generatedID = String.format("U%03d", store.getNextUserID());
                    	store.addUser(uname, pwd, role);
                ...
                case 3:
                	...
                	store.getUserList().forEach(u -> System.out.println("-" + u.getUserID() + ": " + u.getUsername()));
                	
                	System.out.print("Enter UserID to remove: ");
                	String userIDToRemove = sc.nextLine().trim();
                	
                	if (userIDToRemove.equals(currentUser.getUserID())) {
                		System.out.println("Error: Cannot remove this user!");
                		break;
                	}
                	
                	if (store.removeUser(userIDToRemove)) {
                		System.out.println("User " + userIDToRemove + " removed successfully.");
                	} else {
                		System.out.println("Error: Cannot remove this user!");
                	}
                	break;
        ...
        do {
            ...
            switch (choice) {
                ...
                case 5:
                	...
                	double newP = Double.parseDouble(sc.nextLine().trim());
                	System.out.print("New Selling Price: ");
                	double newS = Double.parseDouble(sc.nextLine().trim());
                	if (store.getInventory().updatePrice(pid2, newP, newS)) {
        ...
        double sp = Double.parseDouble(sc.nextLine().trim());
        	switch (type) {
        		case "BOOK":
        			...
        			String bookId = String.format("B%03d", store.getNextBookID());
        			store.addProduct(new Book(bookId,name, qty, pp, sp, pub, auth, isbn));
        			break;
        		case "STATIONARY":
        			...
        			String stationaryId = String.format("S%03d", store.getNextStationaryID());
        			store.addProduct(new Stationary(stationaryId, name, qty, pp, sp, brand, stype));
        			break;
        		case "TOY":
        			System.out.print("Brand: ");
        			...
        			String toyId = String.format("T%03d", store.getNextToyID());
        			store.addProduct(new Toy(toyId, name, qty, pp, sp, tbrand, age));
        			break;
        		default:
        ...
        do {
            ...
            switch (choice) {
                case 1:
                    ...
                    CustomerInfo customer = store.findOrCreateCustomerInfo(cname, cinfo);
                    Order order = store.createOrder(LocalDate.now(), customer);
                    
                    System.out.println("Created Order with OrderID: " + order.getOrderID());
                    ...
                    while (true) {
                        ...
                        store.addItemToOrder(order.getOrderID(), pid2, qty2);
                    }
                    ...
                    String invoiceID = "INV" + order.getOrderID();
                    Invoice inv = store.generateInvoice(invoiceID, LocalDate.now(), order.getOrderID(), payMethod);
                    if (inv != null) {
                        System.out.println("\n=== Invoice Generated ===");
                ...
                case 2:
                    for (Order o : store.getOrderList()) {
                    	CustomerInfo customerInfo = o.getCustomerInfo();
                        ...
                        System.out.println("");
                        System.out.printf("    + Customer: %s (ID: %s, Contact: %s)", customerInfo.getName(), customerInfo.getCustomerID(), customerInfo.getContactInfo());
                        System.out.println("");
                    }
                    break;
                case 0: break;
